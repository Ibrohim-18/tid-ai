...
const DraggableText: React.FC<DraggableTextProps> = ({ element, onPositionChange, style, canvasRef, isDraggingClass, highlightedWords, id, setDraggingElement }) => {
  const elemRef = useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    const el = elemRef.current;
    const canvasEl = canvasRef.current;
    // Use x < 0 as a sentinel for initial auto-positioning
    if (el && canvasEl && element.position.x < 0) {
      // Hide element briefly to prevent flash of uncentered content
  ...
  return (
    <div
      ...
      style={{
        ...
        width: 'fit-content',
        maxWidth: '90%',
        padding: '12px', // Use explicit pixel padding for consistent rendering
...
const Sticker: React.FC<{ sticker: StickerElement, onUpdate: Function, onRemove: Function, onSelect: Function, isActive: boolean }> = ({ sticker, onUpdate, onRemove, onSelect, isActive }) => {
    ...
    return (
        <div ref={ref}
            ...
            onMouseDown={(e) => handleInteraction(e, 'drag')}
            onTouchStart={(e) => handleInteraction(e, 'drag')}
            style={{
            ...
            {isActive && <>
              ...
              <div className="sticker-control absolute w-4 h-4 bg-white border-2 border-indigo-600 rounded-full cursor-nwse-resize" style={{ bottom: controlOffset, right: controlOffset, zIndex: sticker.zIndex + 1}} onMouseDown={(e) => handleInteraction(e, 'resize')} onTouchStart={(e) => handleInteraction(e, 'resize')} />
              <div className="sticker-control absolute w-4 h-4 bg-white border-2 border-indigo-600 rounded-full cursor-alias" style={{ top: controlOffset, right: controlOffset, zIndex: sticker.zIndex + 1}} onMouseDown={(e) => handleInteraction(e, 'rotate')} onTouchStart={(e) => handleInteraction(e, 'rotate')} />
              <div className="sticker-control absolute w-4 h-4 bg-red-500 text-white text-xs flex items-center justify-center font-bold border-2 border-white rounded-full cursor-pointer" style={{ top: controlOffset, left: controlOffset, zIndex: sticker.zIndex + 1 }} onClick={(e) => { e.stopPropagation(); onRemove(sticker.id); }}>&#x2715;</div>
            </>}
        </div>
    );
};
...
const CanvasArea = forwardRef<HTMLDivElement, CanvasAreaProps>((props, ref) => {
  const { ayah, setAyah, translation, setTranslation, textColor, bgColor, backgroundMode, highlightedWords, draggingElement, setDraggingElement, stickers, onUpdateSticker, onRemoveSticker, activeElementId, setActiveElementId, onSelectSticker } = props;

  const handleAyahPosChange = useCallback((pos) => {
    setAyah(a => ({...a, position: pos}));
  }, [setAyah]);

  const handleTranslationPosChange = useCallback((pos) => {
    ...
    <div id="canvas" ref={ref} className={canvasClasses.join(' ')} style={canvasStyles} onMouseDown={handleCanvasClick}>
      ...
      {stickers.map(sticker => (
          <Sticker
              ...
              isActive={sticker.id === activeElementId}
          />
      ))}
    </div>
  );
