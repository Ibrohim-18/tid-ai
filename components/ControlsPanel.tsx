...
const Popover: React.FC<{ title: string; children: React.ReactNode; onClose: () => void }> = ({ title, children, onClose }) => (
  <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 w-[calc(100vw-32px)] max-w-[360px] bg-white/90 backdrop-blur-lg rounded-xl p-5 shadow-2xl z-20" onClick={e => e.stopPropagation()}>
    <div className="flex justify-between items-center mb-4">
      ...
      <button onClick={onClose} className="text-gray-400 hover:text-gray-800 font-bold text-2xl" aria-label="Close panel">&times;</button>
    </div>
...
const ToolbarButton: React.FC<{ onClick: (e: React.MouseEvent) => void; isActive: boolean; children: React.ReactNode; }> = ({ onClick, isActive, children }) => (
    <button
      onClick={onClick}
      className={`px-3 py-2 text-sm font-medium rounded-md transition-colors ${isActive ? 'bg-indigo-600 text-white shadow' : 'bg-white/50 hover:bg-white/80'}`}
    >
      {children}
    </button>
);
                  ...
                  <div className="p-2 bg-gray-50 rounded-lg space-y-2 max-h-32 overflow-y-auto mb-2 border">
                    {props.customFonts.length > 0 ? props.customFonts.map(font => (
                      <div key={font.familyName} className="flex justify-between items-center text-sm bg-gray-200/70 p-2 rounded">
                        ...
                        <button onClick={() => props.onRemoveCustomFont(font.familyName)} className="text-gray-400 hover:text-red-500 font-bold text-xl ml-2 focus:outline-none flex-shrink-0" title={`Remove ${font.name}`}>&times;</button>
                      </div>
             ...
             <Popover title="Stickers" onClose={() => setActiveMenu(null)}>
                <div className="space-y-4">
                    <div className="grid grid-cols-4 gap-3 p-2 bg-gray-50 rounded-lg max-h-48 overflow-y-auto border">
                        {props.userStickers.length > 0 ? (
                            props.userStickers.map((stickerSrc, index) => (
                                <div key={index} className="relative group aspect-square">
                                    <button
                                        onClick={() => props.onAddSticker(stickerSrc)}
                                        className="w-full h-full bg-white rounded-md p-1 hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition shadow-sm"
                                        title="Add this sticker to canvas"
                                    >
                                        <img src={stickerSrc} alt={`User sticker ${index + 1}`} className="w-full h-full object-contain" />
                                    </button>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); props.onRemoveUserSticker(stickerSrc); }}
                                        className="absolute top-0 right-0 -mt-1 -mr-1 w-5 h-5 bg-red-500 text-white rounded-full text-xs font-bold flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 focus:opacity-100 focus:outline-none"
                            ...
                            <div className="col-span-4 text-center text-sm text-gray-500 py-4">
                                Your uploaded stickers will appear here.
                            </div>
                        )}
                    </div>
                    <div>
                        <label htmlFor="stickerUpload" className="w-full block text-center p-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition text-indigo-500 font-medium">
                            📤 Upload New Sticker
                        </label>
