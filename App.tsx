...
import { Language, BackgroundMode, CustomFontTarget } from './types';
...
const App: React.FC = () => {
  const [appState, setAppState] = useState<AppState>(() => {
    ...
    try {
      ...
      if (savedState) {
        ...
        // Basic check to ensure it's not totally broken
  ...
  useEffect(() => {
    ...
    const fontFaces = appState.customFonts.map(font => `
        @font-face {
  ...
  const handleStickerUpload = useCallback((src: string) => {
    ...
    img.onload = () => {
        ...
        setAppState(s => {
            ...
            const newUserStickers = s.userStickers.includes(src) 
                ? s.userStickers 
                : [...s.userStickers, src];
  ...
  const handleUpdateSticker = useCallback((id: string, newProps: Partial<StickerElement>) => {
      setAppState(s => ({
          ...
          stickers: s.stickers.map(sticker =>
              sticker.id === id ? { ...sticker, ...newProps } : sticker
          )
      }));
  }, []);
  ...
  const handleDownloadImage = useCallback((format: 'png' | 'jpeg') => {
    ...
    // Get precise dimensions from the element to ensure the export matches the screen
    ...
    html2canvas(canvasElement, {
        ...
        if (format === 'jpeg') {
            ...
            if (ctx) {
                // For JPEG, we need to draw it on a canvas with a solid background
  ...
  return (
    <div className="font-sans text-gray-800 w-screen h-[100svh] overflow-hidden bg-black">
       ...
       <style>{`
        ...
        #canvas.exporting .draggable-text-wrapper { /* During export, ensure text elements don't show any transforms */
            transform: none !important;
        }
        #canvas.exporting .sticker-control, #canvas.exporting .sticker-border {
